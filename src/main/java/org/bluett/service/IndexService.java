...
public class IndexService {
    ...
    public ObservableList<TestSuiteVO> selectTestSuiteVOList(TestSuite testSuite, Page page) {
        try(SqlSession session = DatabaseHelper.getSqlSession()){
            ...
            List<TestCase> testTestCaseList = testCaseMapper.selectTestCaseBySuiteIds(suiteIdList);
            suiteVOObservableList.forEach(suiteVO -> {
                ObservableList<TestCaseVO> caseVOObservableList = testTestCaseList.stream()
                        .filter(testTestCase -> suiteVO.getId() == testTestCase.getSuiteId())
    ...
    public TestSuiteVO saveTestSuiteVO(TestSuiteVO testSuiteVO) {
        try(SqlSession session = DatabaseHelper.getSqlSession()) {
            ...
            // 保存testCase
            List<TestCase> testTestCaseList = new ArrayList<>();
            testSuiteVO.getTestCaseList().forEach(testCaseVO -> testTestCaseList.add(BeanUtil.copyProperties(testCaseVO, TestCase.class)));
            TestCaseMapper testCaseMapper = session.getMapper(TestCaseMapper.class);
            testCaseMapper.insertBatch(testTestCaseList);
            session.commit();
            return testSuiteVO;
        }catch (Exception e){
